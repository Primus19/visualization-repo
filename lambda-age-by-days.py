...
# Replace NaN values in 'age_in_days' with a placeholder, e.g., -1, before converting to integers
df['age_in_days'] = df['age_in_days'].fillna(-1).astype(int)

# Extract image name and version
df['image_name'] = df['container_image'].apply(lambda x: x.strip('[]').split('/')[-1].split(':')[0])
df['image_version'] = df['container_image'].apply(lambda x: x.strip('[]').split(':')[-1])
